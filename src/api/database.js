...
export default class Database {
  ...
  async prepareDatabase() {
    try {
      ...
      Database.notificationsDB = new Low(new MemorySync());

      try {
        await Database.interfaceDB.read();
      } catch (error) {
        log.error(`Lỗi khi đọc file database: ${error.message}`, 'Database', 'database');
        // Khởi tạo dữ liệu mặc định nếu không đọc được
      ...
      try {
        // Lưu luôn dữ liệu mới (với widgets.items rỗng) vào file database
        await Database.interfaceDB.write();
      } catch (writeError) {
        log.error(`Lỗi khi ghi file database: ${writeError.message}`, 'Database', 'database');
      }
      ...
      log.error(`Lỗi khi khởi tạo database: ${error.message}`, 'Database', 'database');
      // Khởi tạo các đối tượng cơ bản để ứng dụng có thể chạy
      Database.interfaceDB = Database.interfaceDB || new Low(new JSONFile(Database.databaseFilePath));
      Database.tokensDB = Database.tokensDB || new Low(new MemorySync());
      Database.recordingsDB = Database.recordingsDB || new Low(new MemorySync());
      Database.notificationsDB = Database.notificationsDB || new Low(new MemorySync());
  ...
  static async writeConfigCamerasToDB() {
    ...
    for (const cam of ConfigService.ui.cameras) {
      ...
      // Kiểm tra và thêm uid cho widget camera nếu chưa có
      const widgetItems = await Database.interfaceDB.chain.get('settings').get('widgets').get('items').value();
      for (const item of widgetItems) {
        if (item.type === 'CamerasWidget' && !item.uid) {
          item.uid = `widget-${Date.now()}-${Math.random().toString(36).slice(2, 11)}`;
        }
      }
    }
