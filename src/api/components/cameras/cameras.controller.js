...
export const getCamerasList = async (req, res) => {
  try {
    const allCameras = await CamerasModel.list();
    const camerasSettings = await Database.interfaceDB.chain.get('settings').get('cameras').cloneDeep().value();

    // Lấy danh sách rooms từ cấu hình
    const rooms = await Database.interfaceDB.chain.get('settings').get('general').get('rooms').cloneDeep().value() || ['Standard'];

    // Tạo cấu trúc dữ liệu phân loại theo room
    const camerasByRoom = {};
    rooms.forEach(room => {
      camerasByRoom[room] = allCameras
        .filter(camera => {
          const cameraSetting = camerasSettings.find(cs => cs.name === camera.name);
          return (cameraSetting?.room || 'Standard') === room;
        })
        .map(camera => ({
          name: camera.name,
          rtsp_link: camera.videoConfig?.source.replace(/^-i\s+/i, '') || ''
        }));
    });

    res.status(200).send(camerasByRoom);
  } catch (error) {
    console.log(error);
