...
.tw-w-full.tw-mt-8
  ...
  .tw-mb-5.tw-mt-5(v-if="!loading")
    ...
    .tw-mt-4.tw-mb-8(v-for="(user,i) in users" :key="user.username" v-if="users.length > 1 && !user.permissionLevel.includes('admin')")
      ...
      .tw-flex.tw-items-center.tw-w-full.tw-mb-2
        .tw-flex-grow
          label.form-input-label {{ $t('username') }}
          v-text-field(disabled :value="user.username" prepend-inner-icon="mdi-account" background-color="var(--cui-bg-card)" color="var(--cui-text-default)" solo)
            template(v-slot:prepend-inner)
              v-icon.text-muted {{ icons['mdiAccount'] }}
        .tw-ml-2.tw-flex.tw-items-center
          v-btn(icon color="error" @click="remove(user, i)")
...
export default {
  ...
  async created() {
    try {
      const users = await getUsers('?pageSize=1000');
