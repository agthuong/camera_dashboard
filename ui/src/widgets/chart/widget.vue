...
export default {
  ...
  methods: {
    ...
    handleIframeLoad() {
      ...
      try {
        const iframe = this.$refs.chartIframe;
        if (iframe) {
          // Kiểm tra nếu có thể truy cập contentDocument
          if (iframe.contentDocument) {
            console.log('Có thể truy cập nội dung iframe');
          }
        }
      } catch (error) {
        // Lỗi truy cập content do CORS, nhưng iframe vẫn tải được
        console.log('Không thể truy cập nội dung iframe do CORS, nhưng iframe vẫn tải được');
      }
    },
