...
.tw-h-full
  ...
  .video-card-container(ref="videoCardContainer" v-click-outside="{ handler: closeFullscreen, include: include }")
    ...
    v-card.video-card.card.tw-flex.tw-flex-col.fill-height.tw-relative(ref="videoPlayer" :class="(blank ? 'no-radius ' : ' ') + (fullscreen ? 'tw-h-full' : '')")
      ...
      // Video Title (top)
      .tw-z-10(v-if="title && titlePosition === 'top' && !fullscreen" @click="!preventTitleClick ? $router.push(`cameras/${camera.name}`) : null")
        v-card-title.video-card-top-title.tw-flex.tw-justify-between.tw-items-center(:class="{'tw-cursor-pointer': !preventTitleClick}")
      ...
      .tw-z-10.tw-flex.tw-justify-center.tw-items-center.tw-w-full.tw-relative(v-if="title && titlePosition === 'inner-top' && !fullscreen")
        .video-card-inner-top-title {{ camera.name }}
...
export default {
  ...
  methods: {
    ...
    async startStream() {
      try {
        ...
        if (status.data.status === 'ONLINE') {
          ...
          this.timeout = this.camera.settings.streamTimeout || 10;
          ...
          this.player = new JSMpeg.Player(null, {
            ...
            onSourceError: (error) => {
              ...
              this.loading = false;
              this.offline = true;
              this.$toast.error(`${this.camera.name}: Lỗi stream - ${error}`);
            },
          });
    ...
    stopStream() {
      ...
      // Dọn dẹp socket listener khi dừng stream
      this.$socket.client.off(this.camera.name, this.writeStream);
