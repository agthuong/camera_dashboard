<template lang="pug">
  .widgets-navi.pr-safe.tw-flex.tw-flex-col.tw-h-full(
    :class="(showSidebar ? 'widgets-navi-show ' : '')"
    v-click-outside="{ handler: hideNavi, include: include }"
  )
    .widgets.tw-p-3
      ...
      .widget(
        ...
        :class="i !== widgets.length - 1 ? 'tw-mb-10' : ''"
      )
        .widget-title.tw-mb-2.tw-ml-2 {{ widget.name }}
        component(
          :is="widget.placeholderComponent"
          :items="items"
          :dataset="widget.defaultWidgetData"
          :widgets="widget.items"
          :type="widget.type"
          @refreshDrag="$emit('refreshDrag')"
          @widgetDropped="handleWidgetDropped(widget)"
        )
</template>
...
export default {
    ...
    return {
      ...
      widgets: [
        ...
        },
        {
          ...
          items: [{ id: 'freeSpace' }, { id: 'usedByRecordings' }],
        },
        {
        ...
        },
        {
          ...
          items: [{ id: 'cpuLoad' }, { id: 'cpuTemperature' }, { id: 'memory' }],
        },
        {
          type: 'ConsoleWidget',
          name: this.$t('console'),
  ...
  methods: {
    ...
    handleWidgetDropped(widget) {
      // Tạo UID duy nhất cho widget mới
      const uid = `widget-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
      const widgetWithUID = {
        ...widget,
        uid,
        id: uid, // Đảm bảo id và uid giống nhau để tránh xung đột
      };
      this.$emit('addWidget', widgetWithUID);
    },
    hideNavi() {
